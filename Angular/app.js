      ...
      Celda = function(salida, llegada, costo) {
        ...
        $scope.$watch(
          ...
          function() {
            ...
            //Si hay una forma de crear el de Dijkstra, lo Creamos
            if (!(typeof $scope.selectedSalidaDij === "undefined" || $scope.selectedLlegadaDij === "undefined")) {
              $scope.dijkstra_init($scope.selectedSalidaDij, $scope.selectedLlegadaDij, $scope.vertices, $scope.matriz)
            }

            //Creamos el arbol de Kruskal si cambia una conexion
            var ceros = 0;
            for (var i = 0; i < $scope.vertices.length; i++) {
              //total += $scope.vertices[i].grado;
              if ($scope.vertices[i].grado === 0) {
                  ceros++;
                }
              }

              if (ceros === 0) {
                $scope.kruskal_init($scope.vertices, $scope.matriz, true);
              }
        ...
        $scope.kruskal_init = function(vertices, conexiones, minimo) {
          ...
          for (var i = 0; i < escogidos.length; i++) {
            costoTotal += escogidos[i];
          }

          template = "Costo total: " + costoTotal + "<br/>"
        ...
        function getArbol(escogidos, c_original) {
          ...
          while (escogidos.length !== 0) {
            ...
            arbol = arbol.filter(function(item) {
              return !isHijo(item.conexion, hijosFull);
            });
            //alert("Escogidos: " + escogidos.length);
